...
class RtcActiveRoomScreen extends StatefulWidget {
  final String roomId;
  final String? roomPassword;

  const RtcActiveRoomScreen({
    super.key,
  ...
  @override
  State<RtcActiveRoomScreen> createState() => _RtcActiveRoomScreenState();
}

class _RtcActiveRoomScreenState extends State<RtcActiveRoomScreen> with WidgetsBindingObserver {
  final _roomService = RtcRoomService();
  final _screenShareChannel = MethodChannel('com.starlight.console/screen_share');
  ...
  String _controlType = 'all_access';
  bool _isCreator = false;
  String _myUserId = '';

  // WebRTC Mesh connections: user_id -> PeerConnection
  final Map<String, RTCPeerConnection> _peerConnections = {};

  // Remote Media Renderers: user_id -> RTCVideoRenderer
  final Map<String, RTCVideoRenderer> _remoteRenderers = {};

  // Participants list (user_id -> RtcParticipant)
  final Map<String, RtcParticipant> _participants = {};

  // Active Speakers (set by server via active_speakers messages)
  ...
  // Pinned user — their video stays in main area regardless of who's speaking
  String? _pinnedUserId;
  Timer? _waveAnimTimer;

  // Audio level sender timer (sends level to server every 200ms)
  Timer? _vadTimer;
  // Local speaking state (driven by server's active_speakers, not local detection)
  bool _wasTalkingLastFrame = false;

  // Screen sharing state
  bool _isScreenSharing = false;
  MediaStream? _screenShareStream;
  MediaStreamTrack? _savedVideoTrack;

  // Fast (~100ms) audio level reader for real-time display + C++ FFT bridge
  ...
  // Exponentially-smoothed band values for visual smoothness
  ...
  // Button visibility helpers based on room_type
  bool get _canMic {
    if (_isCreator) return true;
    return _roomType != 'one_speaker';
  }

  bool get _canCam {
    if (_isCreator) return true;
    return _roomType == 'all_speakers_all_videos';
  }
  ...
  // Buffer for ICE candidates received before PC is created
  ...
  Future<void> _cleanup() async {
    ...
    _stopScreenShare();
    _screenShareChannel.invokeMethod('stopService');
    _socket?.sink.close();
  ...
  Future<void> _reconnectWebSocket() async {
    ...
    while (_reconnectAttempts < _maxReconnectAttempts && mounted) {
      _reconnectAttempts++;
      try {
        final wsUrl = await _roomService.getSignalingUrl(
          widget.roomId,
  ...
  Future<void> _initAndJoin() async {
    ...
    try {
      // Try audio+video first, fallback to audio-only if camera fails
  ...
  Future<bool> _requestPermissions() async {
    ...
    final ok = statuses[Permission.camera]!.isGranted && statuses[Permission.microphone]!.isGranted;
    return ok;
  }
  ...
  Future<void> _startLocalVadMonitor() async {
    // Send audio level to server every 200ms for server-side speaker detection
    _vadTimer = Timer.periodic(const Duration(milliseconds: 200), (_) => _sendAudioLevelToServer());
    // Fast (~100ms) level reader for local UI animation only
    _fastLevelTimer = Timer.periodic(const Duration(milliseconds: 100), (_) => _readFastLevel());
    _startWaveAnimation();

    // Start native mic level capture for real audio amplitude
    MicLevelChannel.instance.start();
  }

  Future<void> _sendAudioLevelToServer() async {
    if (_localStream == null || _socket == null) return;
    if (!_isMicOn) return;

    // Use the real mic level from MicLevelChannel (Android AudioRecord → JNI → C++ FFT)
    ...
    // Send to server — server decides who's speaking
  ...
  Future<void> _readFastLevel() async {
    ...
    // Primary: real mic audio via Android AudioRecord → JNI → C++ FFT
    double level = MicLevelChannel.instance.currentLevel;

    // Fallback: mic enabled
    if (level <= 0) {
    ...
    // Debug log every ~1s
    if (_fastLevelSampleCount % 10 == 0) {
      debugPrint('🎤 FAST_LEVEL: ${(level * 100).toStringAsFixed(1)}% | isSpeaking: ${_activeSpeakerIds.contains(_myUserId)} | peerPCs: ${_peerConnections.length}');
    }
  }
  ...
  void _startWaveAnimation() {
    // Listen to real frequency bands from native FFT (via Android AudioRecord → JNI → C++)
    _micDataSubscription = MicLevelChannel.instance.onData.listen((data) {
      ...
      if (data.bands.isNotEmpty) {
        ...
        setState(() {
          ...
          // Apply exponential smoothing for visual smoothness
          ...
          // Overall wave level tracks peak across bands
          _waveLevel += (maxBand - _waveLevel) * 0.15;
        });
      }
    });

    // Fallback timer when no band data arrives yet
    _waveAnimTimer = Timer.periodic(const Duration(milliseconds: 80), (_) {
      if (_nativeAnalyzerReady) return; // MicLevelChannel handles it
      setState(() {
        // Fallback: uses real mic level or binary active speaker state
        final target = (_activeSpeakerIds.isNotEmpty || _wasTalkingLastFrame)
            ? _currentAudioLevel.clamp(0.3, 1.0)
            : 0.0;
  ...
  void _handleSignalingMessage(Map<String, dynamic> data) async {
    ...
    switch (type) {
      case 'room_state':
        // Clean up stale connections from previous session (e.g. after reconnect)
        ...
        setState(() {
          ...
          _controlType = data['control_type'] ?? 'all_access';
          _isCreator = data['is_creator'] ?? false;
        ...
        _startLocalVadMonitor();
        _sendMediaState(); // sync initial state with server
      ...
      case 'user_joined':
        ...
        // The new user initiates offers via room_state; we wait for their signal
        break;
      ...
      case 'user_left':
        ...
        setState(() {
          _participants.remove(leaveId);
          _activeSpeakerIds.remove(leaveId);
        });
      ...
      case 'signal':
        ...
        if (signal['type'] == 'offer') {
          ...
          _socket?.sink.add(jsonEncode({
            'type': 'signal', 'target_id': senderId,
          ...
          if (pc != null) {
            ...
            // Buffer candidate for when PC is created
            _pendingCandidates.putIfAbsent(senderId, () => []).add({
      ...
      case 'active_speakers':
        ...
        setState(() {
          ...
          for (final id in speakerList) {
            if (id is String) _activeSpeakerIds.add(id);
          }
          // Derive local speaking state from server's decision
  ...
  Future<RTCPeerConnection> _createPeerConnection(String peerId, {required bool isOffer}) async {
    ...
    pc.onIceCandidate = (RTCIceCandidate c) {
      ...
      _socket?.sink.add(jsonEncode({
        ...
        'signal': {
          'type': 'candidate', 'candidate': c.candidate,
    ...
    pc.onIceConnectionState = (RTCIceConnectionState state) {
      ...
      if (state == RTCIceConnectionState.RTCIceConnectionStateFailed ||
        ...
        debugPrint('⚠️ ICE connection failed/disconnected for peer: $peerId');
      }
    };
    ...
    // Replay any buffered ICE candidates for this peer
  ...
  void _toggleMic() {
    if (_roomType == 'one_speaker' && !_isCreator) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Only the host can speak in this room.')));
      return;
    }
  ...
  void _toggleCam() {
    ...
    if (hostOnly && !_isCreator) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Only the host can stream video.')));
      return;
    }
  ...
  void _hostMute(String id) {
    final name = _participants[id]?.name ?? 'User';
    _socket?.sink.add(jsonEncode({'type': 'moderation', 'action': 'mute', 'target_id': id}));
    _addLog('You muted $name');
  }
  void _hostStopVideo(String id) {
    final name = _participants[id]?.name ?? 'User';
    _socket?.sink.add(jsonEncode({'type': 'moderation', 'action': 'stop_video', 'target_id': id}));
    _addLog('You stopped $name\'s video');
  }
  void _hostKick(String id) {
    final name = _participants[id]?.name ?? 'User';
    _socket?.sink.add(jsonEncode({'type': 'moderation', 'action': 'kick', 'target_id': id}));
    _addLog('You kicked $name');
  }

  void _hostUnmute(String id) {
    final name = _participants[id]?.name ?? 'User';
    _socket?.sink.add(jsonEncode({'type': 'moderation', 'action': 'unmute', 'target_id': id}));
    _addLog('You unmuted $name');
  }

  void _hostStartVideo(String id) {
    final name = _participants[id]?.name ?? 'User';
    _socket?.sink.add(jsonEncode({'type': 'moderation', 'action': 'start_video', 'target_id': id}));
    _addLog('You started $name\'s video');
  }
  ...
  Future<void> _toggleScreenShare() async {
    ...
    try {
      // Start foreground service required by Android 14+ for MediaProjection
      await _screenShareChannel.invokeMethod('startService');
      ...
      // Save current camera track so we can restore it
      if (_localStream != null) {
        final videoTracks = _localStream!.getVideoTracks();
        if (videoTracks.isNotEmpty) {
          _savedVideoTrack = videoTracks.first;
        }
      }
      ...
      // Update local renderer to show screen
      ...
      // Listen for user clicking "Stop sharing" in system UI
      screenTrack.onEnded = () async {
        if (_isScreenSharing) await _stopScreenShare();
      };
    } catch (e) {
      debugPrint('Screen share cancelled or failed: $e');
      // Stop foreground service if getDisplayMedia failed
      await _screenShareChannel.invokeMethod('stopService');
    }
  }
  ...
  Future<void> _stopScreenShare() async {
    ...
    if (_savedVideoTrack != null) {
      for (final pc in _peerConnections.values) {
        final senders = await pc.getSenders();
        for (final sender in senders) {
          if (sender.track?.kind == 'video') {
            await sender.replaceTrack(_savedVideoTrack);
          }
        }
      }
    }
    ...
    if (_savedVideoTrack != null && _isCamOn) {
      _localRenderer.srcObject = _localStream;
    }
    ...
    setState(() {
      ...
      _savedVideoTrack = null;
    });
    _sendMediaState();
  }

  // ── Mute/Unmute All ──
  bool get _allNonCreatorMuted {
    final nonCreators = _participants.values.where((p) => !p.isCreator).toList();
    if (nonCreators.isEmpty) return false;
    return nonCreators.every((p) => !p.mic);
  }

  void _toggleMuteAll() {
    final allMuted = _allNonCreatorMuted;
    setState(() {
      for (final p in _participants.values) {
        if (!p.isCreator) {
          p.mic = allMuted;
          allMuted ? _hostUnmute(p.userId) : _hostMute(p.userId);
        }
      }
    });
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(allMuted ? 'All participants unmuted.' : 'All participants muted.')),
    );
  }

  void _endRoom() {
    _socket?.sink.add(jsonEncode({'type': 'terminate_room'}));
    Navigator.pop(context);
  }
  ...
  void _showInviteDialog() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            Icon(Icons.person_add, color: Colors.deepPurpleAccent),
            SizedBox(width: 8),
            Text('Invite to Room'),
          ],
        ),
        content: Column(
          ...
          children: [
            const Text('Share this Room ID:', style: TextStyle(color: Colors.grey)),
  ...
  // ── Room Info ──
  void _showRoomInfo() {
    Navigator.pop(context); // close moderation sheet first
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        ...
        content: Column(
          ...
          children: [
            ...
            _infoRow('Room ID', widget.roomId),
            _infoRow('Type', _roomType.replaceAll('_', ' ').toUpperCase()),
            _infoRow('Control', _controlType.replaceAll('_', ' ').toUpperCase()),
            _infoRow('Participants', '${_participants.length + 1}'),
            _infoRow('You', '$_myUserId (Host)'),
          ],
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              const Padding(
                ...
                child: Text('Host Controls', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
              ),
              const SizedBox(height: 8),
              // Per-participant moderation list
              ..._participants.values.map((p) => ListTile(
                    ...
                    subtitle: Row(
                      children: [
                        Icon(p.mic ? Icons.mic : Icons.mic_off, size: 14, color: p.mic ? Colors.green : Colors.red),
                        const SizedBox(width: 6),
                        Icon(p.camera ? Icons.videocam : Icons.videocam_off, size: 14, color: p.camera ? Colors.green : Colors.red),
                      ],
                    ),
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        IconButton(
                          icon: Icon(p.mic ? Icons.mic_off : Icons.mic, size: 18),
                          color: Colors.white70,
                          tooltip: p.mic ? 'Mute' : 'Unmute',
                          onPressed: () {
                            setState(() => p.mic = !p.mic);
                            p.mic ? _hostUnmute(p.userId) : _hostMute(p.userId);
                          },
                        ),
                        IconButton(
                          icon: Icon(p.camera ? Icons.videocam_off : Icons.videocam, size: 18),
                          color: Colors.white70,
                          tooltip: p.camera ? 'Stop video' : 'Start video',
                          onPressed: () {
                            setState(() => p.camera = !p.camera);
                            p.camera ? _hostStartVideo(p.userId) : _hostStopVideo(p.userId);
                          },
                        ),
                        IconButton(
              ...
              // Global actions
              ListTile(
                leading: Icon(_allNonCreatorMuted ? Icons.mic : Icons.mic_off, color: Colors.orange),
                title: Text(_allNonCreatorMuted ? 'Unmute All' : 'Mute All', style: const TextStyle(color: Colors.white)),
                onTap: _toggleMuteAll,
              ),
              ListTile(
                leading: const Icon(Icons.info_outline, color: Colors.blueAccent),
                title: const Text('Room Info', style: TextStyle(color: Colors.white)),
                onTap: _showRoomInfo,
              ),
              ListTile(
                leading: const Icon(Icons.exit_to_app, color: Colors.red),
                title: const Text('End Room', style: TextStyle(color: Colors.red)),
                onTap: () {
                  Navigator.pop(ctx);
                  _confirmEndRoom();
                },
  ...
  void _confirmKick(String userId, String name) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text('Kick $name?'),
        content: const Text('They will be removed from the room.'),
        actions: [
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                subtitle: const Text('You can rejoin later', style: TextStyle(color: Colors.white38, fontSize: 12)),
                onTap: () {
                  Navigator.pop(ctx);
                  _confirmLeaveRoom();
                },
              ...
              if (_isCreator)
                ListTile(
                  ...
                  title: const Text('End Room', style: TextStyle(color: Colors.red)),
                  subtitle: const Text('Ends the call for everyone', style: TextStyle(color: Colors.white38, fontSize: 12)),
                  onTap: () {
                    Navigator.pop(ctx);
                    _confirmEndRoom();
                  },
          ...
          TextButton(
            onPressed: () {
              if (controller.text.trim().toUpperCase() == widget.roomId.toUpperCase()) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Room ID does not match.')),
                );
              }
            },
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('This will end the call for ALL participants.'),
            const SizedBox(height: 8),
            const Text('Type the room ID to confirm:'),
            const SizedBox(height: 4),
            Text(widget.roomId, style: const TextStyle(color: Colors.grey, fontSize: 12)),
  ...
  Widget build(BuildContext context) {
      ...
      return const Scaffold(
        body: Center(child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Connecting to room...', style: TextStyle(color: Colors.grey)),
          ],
        )),
      );
    }

    // Determine who to show in main area and who in PIP strip
    ...
    if (_pinnedUserId != null && _remoteRenderers.containsKey(_pinnedUserId)) {
      // Pinned user always takes main area; speakers still show but as secondary
      displayIds = [_pinnedUserId!];
    } else {
      displayIds = speakerIds.isNotEmpty
          ? speakerIds
          : (hasRemote ? [_remoteRenderers.keys.first] : <String>[]);
    }
    ...
    // Unique key for transition: stable across rebuilds until display set changes
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Main Speaker Area with animated cross-fade
          Positioned.fill(
          ...
          // PIP Strip (non-speaking participants) — animated
          if (hasRemote)
            Positioned(
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Text(
                        'Lv: ${(_currentAudioLevel * 100).toStringAsFixed(1)}%',
                        style: TextStyle(
                          fontSize: 10,
                          color: _wasTalkingLastFrame ? Colors.greenAccent : Colors.white54,
                          fontFamily: 'monospace',
                        ),
                      ),
                    ),
                    const SizedBox(width: 6),
                    if (_nativeAnalyzerReady)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                        decoration: BoxDecoration(
                          color: Colors.black54,
                        ...
                        child: Text(
                          'Freq: ${_frequencyBands.take(6).map((b) => (b * 100).toStringAsFixed(0)).join(",")}...',
                          style: TextStyle(
                            fontSize: 10,
                            color: _wasTalkingLastFrame ? Colors.cyanAccent : Colors.white38,
                            fontFamily: 'monospace',
                          ),
                        ),
                      ),
  ...
  Widget _buildMainArea(List<String> displayIds, {Key? key}) {
    return Container(key: key,
      ...
      child: displayIds.isEmpty
          ...
          : displayIds.length == 1
              ? _buildVideoTile(
                  _participants[displayIds.first]?.name ?? 'Participant',
                  _remoteRenderers[displayIds.first],
                  false,
                  _participants[displayIds.first]?.camera ?? false,
                  _participants[displayIds.first]?.mic ?? true,
                  false,
                  pinned: _pinnedUserId == displayIds.first,
                  userId: displayIds.first,
                )
  ...
  Widget _buildMultiSpeakerGrid(List<String> ids) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        return Column(
          children: List.generate((ids.length / cols).ceil(), (rowIndex) {
            ...
            return Expanded(
              child: Row(
                children: rowItems.map((id) {
                  return Expanded(
                    child: Padding(
                      padding: EdgeInsets.all(rowItems.length > 1 ? 1.5 : 0),
                      child: _AnimatedTile(
                        child: _buildVideoTile(
                      _participants[id]?.name ?? 'Participant',
                      _remoteRenderers[id],
                      false,
                      _participants[id]?.camera ?? false,
                      _participants[id]?.mic ?? true,
                      false,
                      pinned: _pinnedUserId == id,
                      userId: id,
                    ),
                      ),
                    ),
  ...
  Widget _buildVideoTile(
    ...
    final isSpeaking = isLocalTile
        ? _wasTalkingLastFrame
        : (userId != null && _activeSpeakerIds.contains(userId));
          ...
          if (hasVideo && renderer != null)
            ...
            Center(
              child: Column(
                ...
                children: [
                  Container(
                    width: isLocalTile ? 100 : 60,
                    height: isLocalTile ? 100 : 60,
                    decoration: BoxDecoration(
                  ...
                  const SizedBox(height: 8),
                  Text(
                    isLocalTile ? 'You' : name,
                    style: const TextStyle(color: Colors.white70, fontSize: 14, fontWeight: FontWeight.w600),
                  ),
                  if (!isLocalTile)
                    const Text('Camera off', style: TextStyle(color: Colors.white38, fontSize: 10)),
                ],
              ),
            ),
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 4),
                  if (isSpeaking)
                    Icon(Icons.graphic_eq, color: Colors.greenAccent, size: 12),
                ],
              ),
            ),
          ),
  ...
  Widget _buildVoiceWaves() {
    ...
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        if (_activeSpeakerIds.isNotEmpty)
          Text(
            _activeSpeakerIds.length == 1
                ? (_participants[_activeSpeakerIds.first]?.name ?? 'Speaking')
                : '${_activeSpeakerIds.length} people speaking',
        ...
        SizedBox(
          ...
          child: Row(
            ...
            children: List.generate(36, (i) {
              double rawHeight;
              if (_nativeAnalyzerReady) {
                // Freq bins: low→left, high→right; scale to 3-23px
                rawHeight = bands[i] * 20.0 + 3.0;
              } else {
                // Fallback: simulated
                rawHeight = _waveLevel * ((i % 5) / 5.0) * 20.0 + 3.0;
              }
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.deepPurpleAccent.withValues(alpha: 0.5 + (barH / 23.0) * 0.5),
          ...
          Expanded(
            child: Container(
              ...
              child: _liveLogs.isEmpty
                  ? const Center(
                      child: Text('No activity yet', style: TextStyle(color: Colors.white24, fontSize: 9)),
                    )
                  : ListView.builder(
                  ...
                  children: [
                    ...
                    if (_isCreator) ...[
                      ...
                      Container(
                        ...
                        child: const Text('HOST', style: TextStyle(color: Colors.black87, fontSize: 9, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ],
                ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Stack(
                      ...
                      children: [
                        Icon(_isCreator ? Icons.admin_panel_settings : Icons.people, color: Colors.white, size: 20),
                        Positioned(
                          right: -4, top: -4,
                          child: Container(
        ...
        itemBuilder: (context, index) {
          ...
          return _AnimatedTile(
            child: GestureDetector(
              onTap: () {
                setState(() {
                  _pinnedUserId = isPinned ? null : id;
                });
              },
              child: Container(
                width: 80,
                  ...
                  children: [
                    ...
                    if (isPinned)
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: const Icon(Icons.push_pin, size: 8, color: Colors.black87),
                        ),
                      ),
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Flexible(
                              child: Text(name, style: const TextStyle(color: Colors.white, fontSize: 8), overflow: TextOverflow.ellipsis),
                            ),
                            if (_activeSpeakerIds.contains(id))
                              const Padding(
                                padding: EdgeInsets.only(left: 2),
                                child: Icon(Icons.graphic_eq, color: Colors.greenAccent, size: 8),
                              ),
                          ],
                        ),
                      ),
                    ...
                    Positioned(
                      ...
                      child: Container(
                        width: 16, height: 16,
                        decoration: BoxDecoration(
                          color: (part?.mic ?? true) ? Colors.green : Colors.red,
                          shape: BoxShape.circle,
                        ),
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [Colors.transparent, Color(0xFF0F0C20)],
        ),
      ),
      child: SafeArea(
        top: false,
            ...
            children: [
              ...
              if (_canScreenShare) ...[
                _ctrlBtn(
                  ...
                  _isScreenSharing ? Colors.orange : Colors.deepPurpleAccent,
                  _toggleScreenShare,
                ),
                const SizedBox(width: 10),
              ],
              ...
              const SizedBox(width: 10),
              _ctrlBtn(Icons.call_end, Colors.red, _showEndCallSheet, size: 56),
              const SizedBox(width: 10),
              if (_isRecording)
                _buildRecordingPill()
              else
              ...
              if (_isCreator)
                _ctrlBtn(Icons.admin_panel_settings, Colors.amber, _showModerationPanel),
            ],
          ),
        ),
      ),
        ...
        builder: (ctx, setDialogState) {
          ...
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              height: 300,
              child: list.isEmpty
                  ? const Center(child: Text('Only you are in the room.'))
                  : ListView.builder(
                      itemCount: list.length,
                      itemBuilder: (_, i) {
                        final p = list[i];
                        final isSpeaking = _activeSpeakerIds.contains(p.userId);
                        return ListTile(
                          leading: CircleAvatar(
                            backgroundColor: Colors.deepPurpleAccent.withValues(alpha: 0.2),
                            child: Text(p.name[0].toUpperCase(), style: const TextStyle(color: Colors.deepPurpleAccent)),
                          ),
                          title: Row(
                            children: [
                              Flexible(
                              ...
                              if (p.isCreator)
                                const Padding(
                                  ...
                                  child: Text('(Host)', style: TextStyle(color: Colors.orange, fontSize: 11)),
                                ),
                              if (isSpeaking)
                                const Padding(
                                  padding: EdgeInsets.only(left: 6),
                                  child: Icon(Icons.graphic_eq, color: Colors.green, size: 16),
                                ),
                            ],
                          ),
                          subtitle: Row(
                            children: [
                              Icon(p.mic ? Icons.mic : Icons.mic_off, size: 16, color: p.mic ? Colors.green : Colors.red),
                              const SizedBox(width: 8),
                              Icon(p.camera ? Icons.videocam : Icons.videocam_off, size: 16, color: p.camera ? Colors.green : Colors.red),
                            ],
                          ),
                          trailing: SizedBox(
                            width: (_isCreator && _controlType == 'creator_only' && !p.isCreator) ? 140 : 48,
                            child: SingleChildScrollView(
                              scrollDirection: Axis.horizontal,
                              child: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  // Pin/Unpin toggle (everyone can pin)
                                  IconButton(
                                    icon: Icon(
                                      _pinnedUserId == p.userId
                                          ? Icons.push_pin
                                          : Icons.push_pin_outlined,
                                      size: 18,
                                      color: _pinnedUserId == p.userId
                                          ? Colors.amber
                                          : Colors.grey,
                                    ),
                                    onPressed: () {
                                      setState(() {
                                        _pinnedUserId = _pinnedUserId == p.userId
                                            ? null
                                            : p.userId;
                                      });
                                      Navigator.pop(ctx);
                                  ...
                                  if (_isCreator && _controlType == 'creator_only' && !p.isCreator) ...[
                                    IconButton(icon: const Icon(Icons.mic_off, size: 18), onPressed: () => _hostMute(p.userId)),
                                    IconButton(icon: const Icon(Icons.videocam_off, size: 18), onPressed: () => _hostStopVideo(p.userId)),
                                    IconButton(icon: const Icon(Icons.gavel, size: 18, color: Colors.red), onPressed: () {
                                      _hostKick(p.userId);
                                      Navigator.pop(ctx);
                                    }),
                                  ],
...
/// Wraps a child widget with fade+scale entrance animation for smooth tile transitions.
