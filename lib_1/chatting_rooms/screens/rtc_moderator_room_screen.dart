...
class RtcModeratorRoomScreen extends StatefulWidget {
  final String roomId;
  final String? roomPassword;

  const RtcModeratorRoomScreen({
    super.key,
  ...
  @override
  State<RtcModeratorRoomScreen> createState() => _RtcModeratorRoomScreenState();
}

class _RtcModeratorRoomScreenState extends State<RtcModeratorRoomScreen> with WidgetsBindingObserver {
  ...
  // Button visibility — moderator screen is for all_access rooms, no creator restrictions
  bool get _canMic => true;
  bool get _canCam => _roomType == 'all_speakers_all_videos';
  ...
  bool get _canScreenShare => _roomType == 'all_speakers_all_videos';
  ...
  Future<void> _toggleScreenShare() async {
    ...
    try {
      ...
      if (_localStream != null) {
        ...
        if (videoTracks.isNotEmpty) _savedVideoTrack = videoTracks.first;
      }
      for (final pc in _peerConnections.values) {
        final senders = await pc.getSenders();
        for (final sender in senders) {
          if (sender.track?.kind == 'video') await sender.replaceTrack(screenTrack);
        }
      }
      _localRenderer.srcObject = screenStream;
      setState(() { _isScreenSharing = true; _screenShareStream = screenStream; _isCamOn = true; });
      _sendMediaState();
      screenTrack.onEnded = () async { if (_isScreenSharing) await _stopScreenShare(); };
    } catch (e) {
  ...
  Future<void> _stopScreenShare() async {
    ...
    if (_savedVideoTrack != null) {
      for (final pc in _peerConnections.values) {
        final senders = await pc.getSenders();
        for (final sender in senders) {
          if (sender.track?.kind == 'video') await sender.replaceTrack(_savedVideoTrack);
        }
      }
    }
    _screenShareStream?.getTracks().forEach((t) => t.stop());
    await _screenShareChannel.invokeMethod('stopService');
    if (_savedVideoTrack != null && _isCamOn) _localRenderer.srcObject = _localStream;
    setState(() { _isScreenSharing = false; _screenShareStream = null; _savedVideoTrack = null; });
    _sendMediaState();
  }
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () { Navigator.pop(ctx); _confirmLeaveRoom(); },
            ),
            const SizedBox(height: 8),
          ],
        ),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          ...
                          title: Row(
                            children: [
                              Flexible(child: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
                              if (isSpeaking) const Padding(padding: EdgeInsets.only(left: 6), child: Icon(Icons.graphic_eq, color: Colors.green, size: 16)),
                            ],
                          ),
                          subtitle: Row(
                            children: [
                          ...
                          trailing: IconButton(
                            icon: Icon(_pinnedUserId == p.userId ? Icons.push_pin : Icons.push_pin_outlined, size: 18, color: _pinnedUserId == p.userId ? Colors.amber : Colors.grey),
                            onPressed: () {
                              setState(() => _pinnedUserId = _pinnedUserId == p.userId ? null : p.userId);
                              Navigator.pop(ctx);
                            },
                          ),
                        );
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (_waveLevel > 0.05)
            Positioned(left: 0, right: 0, bottom: 165, child: _buildVoiceWaves()),
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Text('Lv: ${(_currentAudioLevel * 100).toStringAsFixed(1)}%',
                        style: TextStyle(fontSize: 10, color: _wasTalkingLastFrame ? Colors.greenAccent : Colors.white54, fontFamily: 'monospace')),
                    ),
                    const SizedBox(width: 6),
                    if (_nativeAnalyzerReady)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                        decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(4)),
                        child: Text('Freq: ${_frequencyBands.take(6).map((b) => (b * 100).toStringAsFixed(0)).join(",")}...',
                          style: TextStyle(fontSize: 10, color: _wasTalkingLastFrame ? Colors.cyanAccent : Colors.white38, fontFamily: 'monospace')),
                      ),
                  ],
                ),
              ),
          ...
          child: Container(
            ...
            child: Stack(clipBehavior: Clip.none, children: [
              ...
              Positioned(right: -4, top: -4, child: Container(
                ...
                child: Text('${_participants.length}', style: const TextStyle(color: Colors.white, fontSize: 8, fontWeight: FontWeight.bold)),
              )),
            ]),
          ),
        ),
        ...
        return _AnimatedTile(child: GestureDetector(
          ...
          child: Container(width: 80, margin: const EdgeInsets.only(right: 8),
            ...
            child: Stack(children: [
              ...
              if (isPinned) Positioned(top: 2, left: 2, child: Container(width: 14, height: 14,
                decoration: const BoxDecoration(color: Colors.amber, shape: BoxShape.circle),
    ...
    return Container(
      ...
      child: SafeArea(top: false, child: SingleChildScrollView(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (_canScreenShare) ...[
            _ctrlBtn(_isScreenSharing ? Icons.stop_screen_share : Icons.screen_share, _isScreenSharing ? Colors.orange : Colors.deepPurpleAccent, _toggleScreenShare),
            const SizedBox(width: 10),
          ],
