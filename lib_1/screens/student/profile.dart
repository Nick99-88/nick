...
import '../setups/parent_connection_screen.dart';
...
class _StudentProfileState extends State<StudentProfile> {
  ...
  String? _studentId;
  String _studentPhone = "Not set";
  String _studentSection = "";
  String _studentJoiningDate = "Not set";
  String? _pfpBase64;
    ...
    try {
      ...
      if (token != null) {
        ...
        if (response.statusCode == 200) {
          ...
          if (mounted) {
            setState(() {
              _studentName = data['full_name'] ?? data['name'] ?? _studentName;
              _studentEmail = data['email'] ?? _studentEmail;
              _studentId = data['public_id'] ?? _studentId;
              _studentPhone = data['phone'] ?? data['phone_number'] ?? _studentPhone;
              _studentSection = data['section_name'] ?? data['designation'] ?? data['department'] ?? '';
              final rawDate = data['joining_date'] ?? '';
              if (rawDate.isNotEmpty) {
                try {
                  final dt = DateTime.parse(rawDate);
                  _studentJoiningDate = '${dt.year}-${dt.month.toString().padLeft(2, '0')}-${dt.day.toString().padLeft(2, '0')}';
                } catch (_) {
                  _studentJoiningDate = rawDate;
                }
              }
            });
          }
        ...
        Container(
          ...
          child: Column(
            children: [
                  ...
                  Positioned(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Icon(Icons.camera_alt, size: 14, color: Colors.green.shade700),
                      ),
                    ),
                  ),
                ],
              ...
              Text(_studentName, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
              const SizedBox(height: 4),
              Text(_studentEmail, style: const TextStyle(fontSize: 13, color: Colors.white70)),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _infoRow(Icons.email, "Email", _studentEmail.isNotEmpty ? _studentEmail : "Not set"),
            _infoRow(Icons.phone, "Phone", _studentPhone),
            if (_studentSection.isNotEmpty)
              _infoRow(Icons.group, "Section", _studentSection),
            _infoRow(Icons.calendar_today, "Joining Date", _studentJoiningDate),
          ],
        ),
      ),
    );
    ...
    return Card(
      ...
      child: Column(
        children: [
          ...
          const Divider(height: 1),
          _menuTile(Icons.family_restroom, "My Parent", "Connect with your parent via Nearby", onTap: () {
            Navigator.push(context, MaterialPageRoute(builder: (_) => const ParentConnectionScreen()));
          }),
          const Divider(height: 1),
