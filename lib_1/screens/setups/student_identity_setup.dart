...
class StudentIdentitySetupScreen extends StatefulWidget {
  const StudentIdentitySetupScreen({super.key});

  @override
  State<StudentIdentitySetupScreen> createState() => _StudentIdentitySetupScreenState();
}

class _StudentIdentitySetupScreenState extends State<StudentIdentitySetupScreen> {
  ...
  final List<String> _grades = [
    ...
    'O-Level', 'A-Level', 'Intermediate', 'Bachelor', 'Master',
  ];
  ...
  Future<void> _handleCompleteProfile() async {
    ...
    try {
      ...
      final payload = {
        ...
        "dob": _dobController.text.isNotEmpty ? _dobController.text : "2005-01-01",
      };

      await _authService.createIdentity(token: token, payload: payload);
      await _authService.initializeRole(token: token, role: "student");
      await StarlightStorage.setIdentity(true);
      await StarlightStorage.setUserRole("student");

      if (mounted) {
        StarlightUtils.showSuccessBox(context, tr('studentIdentitySolidified'));
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                Text(tr('studentIdentity'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: StarlightTheme.primaryBlue)),
                const SizedBox(height: 8),
                Text(tr('fillDetailsToStart'), style: const TextStyle(color: Colors.grey, fontSize: 13)),
                const SizedBox(height: 20),
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      children: [
                        ...
                        _buildTextField(_bioController, tr('aboutMeInterests'), maxLines: 3, prefixIcon: Icons.info_outline),
                        _buildTextField(_dobController, tr('dobField'), hintText: "2005-01-01", prefixIcon: Icons.cake_outlined),
                        _buildGradeDropdown(),
                      ],
                    ),
                  ),
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () {
                      ...
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(builder: (_) => const RoleSelectionScreen(currentRole: "student")),
                      );
                    },
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.school, size: 100, color: StarlightTheme.primaryBlue),
  ...
  Widget _buildTextField(TextEditingController controller, String label, {TextInputType? keyboardType, int maxLines = 1, String? hintText, IconData? prefixIcon}) {
    return Padding(
  ...
  Widget _buildGradeDropdown() {
    return Padding(
      ...
      child: DropdownButtonFormField<String>(
        ...
        dropdownColor: Colors.white,
        items: _grades
            .map((grade) => DropdownMenuItem(value: grade, child: Text(grade, style: const TextStyle(color: Colors.black))))
            .toList(),
        onChanged: (val) => setState(() => _selectedGrade = val),
        decoration: InputDecoration(
          labelText: tr('gradeClass'),
          labelStyle: const TextStyle(color: Colors.black54),
          prefixIcon: const Icon(Icons.class_outlined, color: Colors.black54),
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
          filled: true,
          fillColor: const Color(0xFFF5F5F5),
        ),
        validator: (value) => value == null ? tr('selectGradeRequired') : null,
      ),
    );
  }
}
