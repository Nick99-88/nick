...
class ParentIdentitySetupScreen extends StatefulWidget {
  const ParentIdentitySetupScreen({super.key});

  @override
  State<ParentIdentitySetupScreen> createState() => _ParentIdentitySetupScreenState();
}

class _ParentIdentitySetupScreenState extends State<ParentIdentitySetupScreen> {
  ...
  Future<void> _handleCompleteProfile() async {
    ...
    try {
      ...
      await IdentityController.updateRoleInfo(role: 'parent', roleId: "0");
      await StarlightStorage.setUserRole('parent');
      await StarlightStorage.setIdentity(true);

      if (mounted) {
        StarlightUtils.showSuccessBox(context, tr('parentIdentityCreated'));
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                Text(tr('parentIdentity'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: StarlightTheme.primaryBlue)),
                const SizedBox(height: 8),
                Text(tr('fillParentDetails'), style: const TextStyle(color: Colors.grey, fontSize: 13)),
                const SizedBox(height: 20),
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      children: [
                        ...
                        _buildTextField(_bioController, tr('aboutMe'), maxLines: 3, prefixIcon: Icons.info_outline),
                      ],
                    ),
                  ),
                ),
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () {
                      ...
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(builder: (_) => const RoleSelectionScreen(currentRole: "parent")),
                      );
                    },
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.family_restroom, size: 100, color: StarlightTheme.primaryBlue),
