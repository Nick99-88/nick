...
class _StaffProfileScreenState extends State<StaffProfileScreen> {
  ...
  String _staffEmail = "";
  String? _staffId;
  String _staffPhone = "Not set";
  String _staffDesignation = "";
  String _staffJoiningDate = "Not set";
  String? _pfpBase64;
    ...
    try {
      ...
      if (token != null) {
        ...
        if (response.statusCode == 200) {
          ...
          if (mounted) {
            final email = data['email'] ?? _staffEmail;
            if (email.isNotEmpty) await StarlightStorage.setUserEmail(email);
            setState(() {
              _staffName = data['full_name'] ?? data['name'] ?? _staffName;
              _staffEmail = email;
              _staffId = data['public_id'] ?? _staffId;
              _staffPhone = data['phone'] ?? data['phone_number'] ?? _staffPhone;
              _staffDesignation = data['designation'] ?? data['position'] ?? data['role_name'] ?? '';
              final rawDate = data['created_at'] ?? data['joining_date'] ?? '';
              if (rawDate.isNotEmpty) {
                try {
                  final dt = DateTime.parse(rawDate);
                  _staffJoiningDate = '${dt.year}-${dt.month.toString().padLeft(2, '0')}-${dt.day.toString().padLeft(2, '0')}';
                } catch (_) {
                  _staffJoiningDate = rawDate;
                }
              }
            });
          }
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: StarlightTheme.primaryBlue.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 5)),
            ],
          ),
          child: Column(
            children: [
                  ...
                  CircleAvatar(
                    ...
                    child: _pfpBase64 == null
                        ? Text(
                            ...
                            style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: StarlightTheme.primaryBlue),
                          )
                        : null,
                  ),
                  Positioned(
              ...
              Text(_staffName, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
              const SizedBox(height: 4),
              Text(_staffEmail, style: const TextStyle(fontSize: 13, color: Colors.white70)),
              const SizedBox(height: 8),
              GestureDetector(
                onTap: () {
                  if (_staffId != null) {
                    Clipboard.setData(ClipboardData(text: _staffId!));
                    ScaffoldMessenger.of(context).showSnackBar(
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        _staffId ?? '',
                        style: const TextStyle(fontFamily: 'monospace', fontSize: 13, fontWeight: FontWeight.bold, letterSpacing: 2, color: Colors.white),
                      ),
                      const SizedBox(width: 8),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text("Profile Information", style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            _infoRow(Icons.email, "Email", _staffEmail.isNotEmpty ? _staffEmail : "Not set"),
            _infoRow(Icons.phone, "Phone", _staffPhone),
            _infoRow(Icons.work, "Position", _staffDesignation.isNotEmpty ? _staffDesignation : "Not assigned"),
            _infoRow(Icons.calendar_today, "Join Date", _staffJoiningDate),
          ],
        ),
      ),
    );
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 3, child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500), textAlign: TextAlign.right)),
        ],
      ),
    );
  }
  ...
  Widget _menuTile(IconData icon, String title, String subtitle, {Color? color, VoidCallback? onTap}) {
    return ListTile(
      ...
      title: Text(title, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 14, color: color ?? Colors.black87)),
