...
/// Unified Call Screen - handles both in-app and out-of-app calls
/// Checks socket connection with ping/pong before signaling
class UnifiedCallScreen extends StatefulWidget {
  final CallSession session;

  const UnifiedCallScreen({super.key, required this.session});
...
class _UnifiedCallScreenState extends State<UnifiedCallScreen>
  ...
  void initState() {
    ...
    _isVideo = _session?.type == CallType.video;
    _peerUserId = _session?.peerUserId;
  ...
  Future<void> _initializeCall() async {
    print('📞 Unified Call: Initializing call for ${_session?.peerName}');

    // Check socket connection with ping/pong
    final socketAlive = await _checkSocketConnection();
    if (!socketAlive) {
      print('📞 Unified Call: Socket not alive, connecting...');
    ...
    // Check if call already transitioned before we mounted
    final currentSession = CallSignalingService.instance.activeCall;
    if (currentSession != null && currentSession.state == CallState.connected) {
      _session = currentSession;
      CallSignalingService.instance.onWebRtcSignal = _handleWebRtcSignal;
      _onCallConnected(currentSession);
    } else {
      CallSignalingService.instance.onWebRtcSignal = _handleWebRtcSignal;
  ...
  Future<bool> _checkSocketConnection() async {
    ...
    print('📞 Unified Call: Checking socket connection with ping/pong...');
    final isAlive = await EnhancedSocketService.pingServer();
    print('📞 Unified Call: Socket alive: $isAlive');
    return isAlive;
  }

  Future<void> _ensureSocketConnected() async {
    if (EnhancedSocketService.isConnected()) {
      print('📞 Unified Call: Socket already connected');
      return;
    }

    try {
      print('📞 Unified Call: Connecting socket...');
      await EnhancedSocketService.connect(source: 'UnifiedCallScreen');
      print('📞 Unified Call: Socket connected successfully');
    } catch (e) {
      print('📞 Unified Call: Socket connection failed - $e');
    }
  }

  void _setupSendChannel() {
    CallSignalingService.instance.setSendViaChatChannel((json) async {
      print('📞 Unified Call: Sending signaling message');
      try {
        if (EnhancedSocketService.isConnected()) {
          EnhancedSocketService.sendRawMessage(json);
        } else {
          print('📞 Unified Call: Socket not connected, ensuring connection...');
          await _ensureSocketConnected();
          if (EnhancedSocketService.isConnected()) {
            EnhancedSocketService.sendRawMessage(json);
          } else {
            print('📞 Unified Call: Failed to connect socket for signaling');
          }
        }
      } catch (e) {
        print('📞 Unified Call: Error sending signaling - $e');
      }
    });
  }
  ...
  void _startIncomingCall() {
    print('📞 Unified Call: Starting incoming call UI');
    print('📞 Unified Call: Hiding CallKit incoming call to prevent duplicate UI');
  ...
  void _startOutgoingCall() {
    print('📞 Unified Call: Starting outgoing call UI');
    _startRingingFeedback();
    _startRingingTimeout();
  ...
  void _onCallConnected(CallSession session) async {
    print('📞 Unified Call: Call connected, initializing WebRTC');
    _stopRingingFeedback();
    _ringingTimeoutTimer?.cancel();
  ...
  Future<void> _initMediaAndCreateOffer() async {
    print('📞 Unified Call: Creating offer');
    try {
      print('📞 Unified Call: Getting user media...');
      _localStream = await _getUserMedia();
      print('📞 Unified Call: Got local stream with ${_localStream!.getAudioTracks().length} audio tracks and ${_localStream!.getVideoTracks().length} video tracks');
      _localRenderer.srcObject = _localStream;
      print('📞 Unified Call: Set local renderer source');

      await _createPeerConnection();
      print('📞 Unified Call: Peer connection created');
      
      _localStream!.getTracks().forEach((track) {
        print('📞 Unified Call: Adding track to peer connection: ${track.kind}');
        _peerConnection!.addTrack(track, _localStream!);
      });

      print('📞 Unified Call: Creating offer with config: $_configMap');
      final description = await _peerConnection!.createOffer(_configMap);
      print('📞 Unified Call: Offer created, setting local description');
      await _peerConnection!.setLocalDescription(description);
      print('📞 Unified Call: Local description set, sending offer signal');
      _sendWebRtcSignal({'type': 'offer', 'sdp': description.sdp});
      print('📞 Unified Call: Offer signal sent');
    } catch (e) {
      print('📞 Unified Call: Error creating offer - $e');
    }
  }

  Future<void> _initMediaAndAnswer() async {
    print('📞 Unified Call: Creating answer');
    try {
      print('📞 Unified Call: Getting user media for answer...');
      _localStream = await _getUserMedia();
      print('📞 Unified Call: Got local stream with ${_localStream!.getAudioTracks().length} audio tracks and ${_localStream!.getVideoTracks().length} video tracks');
      _localRenderer.srcObject = _localStream;
      print('📞 Unified Call: Set local renderer source');
      ...
      print('📞 Unified Call: Tracks added, waiting for offer to create answer');
    } catch (e) {
      print('📞 Unified Call: Error creating answer - $e');
    }
  }
  ...
  Future<void> _createPeerConnection() async {
    ...
    print('📞 Unified Call: Creating peer connection with TURN servers for NAT traversal');
    _peerConnection = await createPeerConnection(config);
    print('📞 Unified Call: Peer connection created');

    _peerConnection!.onIceCandidate = (candidate) {
      print('📞 Unified Call: ICE candidate generated');
      _sendWebRtcSignal({
        'type': 'candidate',
    ...
    _peerConnection!.onIceConnectionState = (state) {
      print('📞 Unified Call: ICE connection state: $state');
      if (state == RTCIceConnectionState.RTCIceConnectionStateConnected) {
        print('📞 Unified Call: ICE connection established - media should flow');
      } else if (state == RTCIceConnectionState.RTCIceConnectionStateFailed) {
        print('📞 Unified Call: ICE connection failed - media will not flow');
      }
    };

    _peerConnection!.onTrack = (event) {
      print('📞 Unified Call: Remote track received: ${event.track.kind}');
      if (event.streams.isNotEmpty) {
        print('📞 Unified Call: Setting remote renderer source');
        _remoteRenderer.srcObject = event.streams[0];
      } else {
        print('📞 Unified Call: WARNING - Remote track has no streams');
      }
    };
    
    _peerConnection!.onConnectionState = (state) {
      print('📞 Unified Call: Peer connection state: $state');
    };
  }

  void _handleWebRtcSignal(Map<String, dynamic> signal) {
    print('📞 Unified Call: Received WebRTC signal: ${signal['type']}');
    print('📞 Unified Call: Signal data: $signal');
    
    if (_peerConnection == null) {
      print('📞 Unified Call: ERROR - Peer connection is null, cannot handle signal');
      return;
    }

    switch (signal['type']) {
      case 'offer':
        print('📞 Unified Call: Handling offer');
        _handleOffer(signal['sdp']);
        break;
      case 'answer':
        print('📞 Unified Call: Handling answer');
        _handleAnswer(signal['sdp']);
        break;
      case 'candidate':
        print('📞 Unified Call: Handling ICE candidate');
        _handleCandidate(signal);
        break;
      default:
        print('📞 Unified Call: Unknown signal type: ${signal['type']}');
    }
  }

  void _handleOffer(String? sdp) async {
    print('📞 Unified Call: _handleOffer called');
    if (sdp == null) {
      print('📞 Unified Call: ERROR - SDP is null');
      return;
    }
    print('📞 Unified Call: Setting remote description (offer)');
    await _peerConnection!.setRemoteDescription(RTCSessionDescription(sdp, 'offer'));
    print('📞 Unified Call: Creating answer with config: $_configMap');
    final answer = await _peerConnection!.createAnswer(_configMap);
    print('📞 Unified Call: Setting local description (answer)');
    await _peerConnection!.setLocalDescription(answer);
    print('📞 Unified Call: Sending answer signal');
    _sendWebRtcSignal({'type': 'answer', 'sdp': answer.sdp});
    print('📞 Unified Call: Answer signal sent');
  }

  void _handleAnswer(String? sdp) async {
    print('📞 Unified Call: _handleAnswer called');
    if (sdp == null) {
      print('📞 Unified Call: ERROR - SDP is null');
      return;
    }
    print('📞 Unified Call: Setting remote description (answer)');
    await _peerConnection!.setRemoteDescription(RTCSessionDescription(sdp, 'answer'));
    print('📞 Unified Call: Remote description set successfully');
  }

  void _handleCandidate(Map<String, dynamic> signal) {
    print('📞 Unified Call: _handleCandidate called');
    print('📞 Unified Call: Candidate data: ${signal['candidate']?.toString().substring(0, 50)}...');
    final candidate = RTCIceCandidate(
      signal['candidate'],
      signal['sdpMid'],
      signal['sdpMLineIndex'],
    );
    print('📞 Unified Call: Adding ICE candidate to peer connection');
    _peerConnection!.addCandidate(candidate);
    print('📞 Unified Call: ICE candidate added');
  }
  ...
  Future<void> _initProximitySensor() async {
    try {
      print('📞 Unified Call: Initializing proximity sensor');
      await ProximityWakeLock.acquire();
      print('📞 Unified Call: Proximity wake lock acquired');
      
      try {
        _proximitySub = _proximityChannel.receiveBroadcastStream().listen((event) {
          final isNear = event as bool;
          if (isNear) {
            _originalVolume = _callVolume;
            _callVolume = 0.0;
          } else {
            _callVolume = _originalVolume;
          }
        });
        print('📞 Unified Call: Proximity sensor listener started');
      } catch (e) {
        print('📞 Unified Call: Proximity sensor channel not available (optional feature) - $e');
        // Proximity sensor is optional, continue without it
      }
    } catch (e) {
      print('📞 Unified Call: Proximity wake lock error - $e');
      // Proximity sensor is optional, continue without it
    }
  }
  ...
  void _acceptCall() {
    print('📞 Unified Call: Accepting call');
    CallSignalingService.instance.acceptCall();
  }

  void _rejectCall() {
    print('📞 Unified Call: Rejecting call');
    CallSignalingService.instance.rejectCall();
  }

  void _endCall() {
    print('📞 Unified Call: Ending call');
    CallSignalingService.instance.endCall();
  }
...
// Simple AudioPlayer placeholder
class AudioPlayer {
  void stop() {}
}

// Simple HapticFeedback placeholder
