...
void main() async {
  ...
  // ── Phase 3: Notification data check from killed-state launch ──
  ...
  // ── Phase 5: Ads (non-blocking, safe on all platforms) ──
  try {
...
class _StarlightEngineGateState extends State<StarlightEngineGate> {
  ...
  Future<void> _checkBiometricLockAndInit() async {
    ...
    final biometricLockEnabled = await StarlightStorage.getBiometricLockEnabled() ?? false;
    
    if (biometricLockEnabled) {
      // Check if enough time has passed since last auth (optional timeout)
      final lastAuthTime = await StarlightStorage.getLastAuthTime() ?? 0;
  ...
  Future<void> _initActivityServices() async {
    ...
    if (_isMobile) {
      ...
      try {
        ...
        CallKitService.instance.setOnCallAcceptedNavigate((session) {
          debugPrint('🏛️ CallKit accept callback for: ${session.callId}');
          final ctx = navigatorKey.currentContext;
          if (ctx != null) {
            Navigator.push(
              ctx,
              MaterialPageRoute(builder: (_) => UnifiedCallScreen(session: session)),
            );
          }
        });
